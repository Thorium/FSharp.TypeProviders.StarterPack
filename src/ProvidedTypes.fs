...
module internal AssemblyReader =
    ...
    module Utils =
        ...
        let sha1HashBytes (s: byte[]) = 
            use sha = System.Security.Cryptography.SHA1.Create()
            let fullHash : byte[] = sha.ComputeHash(s)
            // Extract h3 and h4 from the last 8 bytes of SHA1 hash (equivalent to custom implementation)
            let h3 = (int32 fullHash.[12] <<< 24) ||| (int32 fullHash.[13] <<< 16) ||| (int32 fullHash.[14] <<< 8) ||| (int32 fullHash.[15])
            let h4 = (int32 fullHash.[16] <<< 24) ||| (int32 fullHash.[17] <<< 16) ||| (int32 fullHash.[18] <<< 8) ||| (int32 fullHash.[19])
            [| b0 h4; b1 h4; b2 h4; b3 h4; b0 h3; b1 h3; b2 h3; b3 h3 |]
